...
void InteractiveViewerWidget::draw_selected_edge()
{
	if (is_show_papamaterization)
	{
        ...
        */
        ...
        if(!graph_init)
        {
            graph = new EdgeToGraph(cut_mesh);

            graph_init = true;
        }

        //graph->draw_all_edges(255, 255, 255);

        //graph->draw_all_edges_on_bound(255, 255, 255);

        //graph->draw_edges_on_bound(255, 255, 255);

        graph->draw_color_on_total_graph();

        graph->show_points();

        //graph->show_idx_of_points();

        graph->draw_surface_map(255, 255, 255);

        graph->show_idx_of_surface_map();
	}
}
...
void  InteractiveViewerWidget::LoadFace(bool property_added)
{	
    if (1)
	{
        ...
        if(png_idx == 0)
        {
            filename=QFileDialog::getOpenFileName(0,tr(""),"./Samples","txt (*.txt);;SVG (*.svg)");
        }
        else if(png_idx == 1)
        {
            filename = QString("./Samples/he.txt");
        }
        else if(png_idx == 2)
        {
            filename = QString("./Samples/ma.txt");
        }
        else if(png_idx == 3)
        {
            filename = QString("./Samples/niao.txt");
        }
        else if(png_idx == 4)
        {
            filename = QString("./Samples/shu.txt");
        }
        else if(png_idx == 5)
        {
            filename = QString("./Samples/tu.txt");
        }
        else if(png_idx == 6)
        {
            filename = QString("./Samples/wa.txt");
        }
        else if(png_idx == 7)
        {
            filename = QString("./Samples/xia.txt");
        }
        else if(png_idx == 8)
        {
            filename = QString("./Samples/xie.txt");
        }
        else if(png_idx == 9)
        {
            filename = QString("./Samples/ying.txt");
        }

        QFileInfo info(filename);
        if(filename.toStdString()=="")
        {
            cout << "You do not choose any file, Please choose again." << endl;
            return;
        }
...
void InteractiveViewerWidget::comput_cover_area(Mesh& my_mesh, double& area_r, double& area_g)
{
	...
	{
        Mesh::FaceVertexIter fv_it = my_mesh.fv_begin(*f_it);

        Mesh::Point p0 = my_mesh.point(*fv_it);
...
void InteractiveViewerWidget::GetCut()
{
	...
	ZipUp zip_tool;
    bool read_OK = OpenMesh::IO::read_mesh(zip_tool.mesh_, "output/obj/Cut_Result.obj");
...
void InteractiveViewerWidget::Parameterzation_to_OBJ()
{
	if (!NO_Papamaterization)
	{
        ...
        OpenMesh::IO::write_mesh(ref_mesh, "output/obj/Ref_Mesh.obj");
        ...
        OpenMesh::IO::write_mesh(curved_mesh, "output/obj/Curved_Mesh.obj");
	}
}
void InteractiveViewerWidget::ARAPresult_to_OBJ()
{
    ...
    OpenMesh::IO::write_mesh(mesh, "output/obj/Cut_Result.obj");
...
void InteractiveViewerWidget::boundary_poly_to_file()
{
    ...
    std::ofstream file("output/boundary/Boundary.txt");
